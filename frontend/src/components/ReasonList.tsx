"use client";

import { useState } from "react";
import Icon, { IconName } from "@/components/Icon";

/**
 * จัดหมวดหมู่เหตุผล (reason) ของ signal card — แต่ละหมวด toggle พับ/กางได้
 * หมวดอิงจาก keyword ที่ StrategyEngine ใช้เขียน reason (ภาษาไทย) จริง:
 *   trend     — EMA / ADX / เทรนด์ / Supertrend
 *   momentum  — RSI / MACD / โมเมนตัม
 *   volatility— ATR / ผันผวน / volatility
 *   news      — ข่าว / impact / sentiment
 *   score     — Opportunity Score (หัวข้อแรกที่ build_proposal ใส่)
 *   other     — ที่เหลือ
 */
type Category = "score" | "trend" | "momentum" | "volatility" | "news" | "other";

const CATEGORY_META: Record<Category, { label: string; icon: IconName; color: string }> = {
  score: { label: "คะแนนโอกาส", icon: "target", color: "text-accent" },
  trend: { label: "เทรนด์", icon: "trendUp", color: "text-profit" },
  momentum: { label: "โมเมนตัม", icon: "bolt", color: "text-amber-400" },
  volatility: { label: "ความผันผวน", icon: "waves", color: "text-sky-400" },
  news: { label: "ข่าว/เซนติเมนต์", icon: "news", color: "text-purple-400" },
  other: { label: "อื่นๆ", icon: "help", color: "text-slate-400" },
};

const CATEGORY_ORDER: Category[] = ["score", "trend", "momentum", "volatility", "news", "other"];

function classify(reason: string): Category {
  const r = reason.toLowerCase();
  if (r.includes("opportunity score")) return "score";
  // momentum ก่อน trend: เหตุผล RSI/Supertrend ผสม ("RSI ... + Supertrend ... ในเทรนด์...")
  // มี keyword ทั้งสองฝั่ง — ต้องลงโมเมนตัมก่อน ไม่งั้นเทรนด์กวาดไปหมดเหลือหมวดเดียว
  if (r.includes("rsi") || r.includes("macd") || r.includes("โมเมนตัม") || r.includes("pullback") || r.includes("chase") || r.includes("overbought") || r.includes("oversold") || r.includes("breakout") || r.includes("retest")) return "momentum";
  if (r.includes("ema") || r.includes("adx") || r.includes("เทรนด์") || r.includes("supertrend")) return "trend";
  if (r.includes("atr") || r.includes("ผันผวน") || r.includes("volatility")) return "volatility";
  if (r.includes("ข่าว") || r.includes("impact") || r.includes("sentiment") || r.includes("เซนติเมนต์")) return "news";
  return "other";
}

export default function ReasonList({ reasons }: { reasons: string[] }) {
  // พับเดียวรวมทุกหมวด (เดิมแยกหัวละหมวด → การ์ดยาว) — กางทีเดียวเห็นครบ
  const [open, setOpen] = useState(false);

  const groups = CATEGORY_ORDER
    .map((cat) => ({ cat, items: reasons.filter((r) => classify(r) === cat) }))
    .filter((g) => g.items.length > 0);

  if (!groups.length) return null;
  const total = groups.reduce((n, g) => n + g.items.length, 0);

  return (
    <div className="mt-2 rounded-xl border border-white/10 bg-white/[0.04]">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between px-2 py-1.5 text-xs hover:bg-white/10 rounded-xl"
        aria-expanded={open}
      >
        <span className="text-slate-400">
          เหตุผล ({total}) — {groups.map((g) => CATEGORY_META[g.cat].label).join(" • ")}
        </span>
        <span className="text-slate-500 shrink-0 ml-2">{open ? "▾" : "▸"}</span>
      </button>
      {open && (
        <div className="px-3 pb-2 space-y-2">
          {groups.map(({ cat, items }) => (
            <div key={cat}>
              <p className={`flex items-center gap-1.5 text-xs font-semibold ${CATEGORY_META[cat].color}`}>
                <Icon n={CATEGORY_META[cat].icon} size={13} />
                {CATEGORY_META[cat].label}
                <span className="text-slate-500">({items.length})</span>
              </p>
              <ol className="list-decimal list-inside mt-0.5 space-y-0.5 text-slate-300 text-xs">
                {items.map((r, i) => <li key={i}>{r}</li>)}
              </ol>
            </div>
          ))}
        </div>
      )}
    </div>
  );
}
