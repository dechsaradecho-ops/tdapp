...
const GUARD_FIELDS: { label: string; key: string; hint: string }[] = [
  ...
  { label: "เลื่อนฉุกเฉิน", key: "emergency_held", hint: "kill switch เข้าเงื่อนไข แต่มีคำขอขยายลิมิตที่รอการยืนยันอยู่ → เลื่อนการปิดไม้ออกไป (SL/TP ยังทำงานปกติ) จนกว่าจะได้คำตอบ หรือครบช่วงยืนยัน (kill_expand_ttl_min) — หมดเวลาแล้วตัดสินแบบ fail-closed ก่อน (Settings → kill_expand_auto_widen: ปิด default = ลิมิตเดิมยังมีผล ไม่ขยายให้เอง แล้วปล่อยให้ kill switch ทำงาน · เปิด = ค่อยไปตามนโยบาย Settings → kill_expand_auto_apply: เปิด = ขยายทุกครั้ง · ปิด = ขยายให้เองได้ครั้งเดียวใน 24 ชม. หลังจากนั้นปล่อยให้ kill switch ปิดไม้) — guard จะปิดไม้เฉพาะเมื่อยังเกินลิมิตใหม่เท่านั้น ถ้าขยายอัตโนมัติทำไม่ได้ (เขียน DB ไม่ได้) จะยังเลื่อนต่อไปโดยไม่ปิดไม้ และเตือนซ้ำทุก ~6 ชม." },
...
function decidedByLabel(by: string): string {
  ...
  if (v === "auto:expired")
    return "ระบบขยายให้อัตโนมัติ (หมดเวลายืนยัน · นโยบายเปิด = ทุกครั้ง)";
  if (v === "auto:once")
    return "ระบบขยายให้เองครั้งเดียวใน 24 ชม. (หมดเวลายืนยัน · นโยบายปิด)";
  if (v === "auto:capped")
    return "ไม่ขยาย — โควตาขยายอัตโนมัติหมด และนโยบายปิดอยู่ (ปล่อยให้ kill switch ทำงาน)";
