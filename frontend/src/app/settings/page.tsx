...
export default function SettingsPage() {
  ...
  const applyPreset = async () => {
    ...
    setPresetMsg("");
    try {
      const table = await api.riskPresets();
      const patch = table[cfg.risk_profile] as Partial<AppSettings> | undefined;
      if (!patch) {
        setPresetMsg(`ไม่รู้จักโปรไฟล์ ${cfg.risk_profile}`);
        return;
      }
      setCfg({ ...cfg, ...patch, risk_profile: cfg.risk_profile });
      setSaveMsg("");
      setPresetMsg("ใส่ค่าตาม preset แล้ว — กด “บันทึกการตั้งค่า” เพื่อลง DB");
    } catch (e) {
