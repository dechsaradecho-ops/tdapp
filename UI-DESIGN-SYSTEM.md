...
⚠️ **"ความยาวแบบด์" คูณทั้งก้อน `clamp()`** — `bandHeight(h) = clamp(360h px, 85h vh, 780h px)` ใน `ScrollZoomHero.tsx` (สไลเดอร์: 50% = h 0.5, 100% = h 1, 200% = h 2) → 100% ให้ `clamp(360px,85vh,780px)` ตรงกับคลาส `h-[clamp(360px,85vh,780px)]` เป๊ะ (inline style ไม่ถูกใส่ตอนค่า = default → คลาสยังเป็นแหล่งความจริงของความสูงดีไซน์); ใส่เป็น inline style ทับคลาสเพราะ Tailwind สร้างคลาสจากค่า static ไม่ได้ · **ต้องคูณทั้งพื้นขั้นต่ำ/ค่า vh/เพดานพร้อมกัน** ไม่งั้นสไลเดอร์จะ "ตายช่วง" (จอสูงเคยติดเพดาน 780px, จอเตี้ยเคยติดพื้น 360px) · เปลี่ยนความยาว = ต้องสร้าง timeline ใหม่ (ผูก `height` เป็น dependency เหมือน `zoom`) เพราะ ScrollTrigger ยึดความสูงของแบนด์ตอน `start: top top` / `end: bottom 25%` — แบนด์ยาวขึ้น ระยะซูมยาวขึ้นเองโดยไม่ต้องแก้ค่า scale ใด ๆ · ถ้าแก้คลาส `h-[clamp(...)]` ต้องแก้ `BAND_MIN/BAND_VH/BAND_MAX` ให้ตรงกัน

⚠️ **การ์ดสไลเดอร์ 3 ตัวของฮีโร่ใช้คอมโพเนนต์ `SliderCard` ตัวเดียว** (label/value/min/max/step/default/readout/hint/onSet) — เพิ่มสไลเดอร์ใหม่ให้ใช้ `SliderCard` ห้ามเขียน markup ซ้ำ (ปุ่ม "ค่าเริ่มต้น" ต้องอยู่ใน `div.rounded-lg` ของสไลเดอร์ตัวเองเสมอ เพราะหน้านี้มีปุ่มข้อความเดียวกันหลายที่)

⚠️ **`.zoom-hero` ต้องมี `z-index: -1` เสมอ — ห้ามเปลี่ยนไปใช้ `relative z-10` ที่ `<main>`**
- `AuthGate` (PIN lock) วาด overlay `fixed inset-0 z-50` **อยู่ "ใน" `<main>`** และ `MobileNav` (`fixed z-40`) กับ FAB ของ `ChatWidget` (`fixed z-40`) อยู่นอก `<main>` → ถ้า `<main>` ได้ `relative z-10` ก็จะกลายเป็น stacking context จับ `z-50` ของ AuthGate ขังไว้ แล้ว dock/FAB (z-40 ที่ root) จะลอยทับหน้าล็อก PIN ทันที
- `z-index: -1` ไม่ได้ทำให้แถบมุดไปใต้ BackgroundLayer เพราะ BackgroundLayer มาก่อนใน DOM (`z-[-2]` รูป, `z-[-1]` scrim) — เท่ากันแล้วตัวที่มาทีหลังชนะ → แถบอยู่เหนือ scrim (ตั้งใจ ไม่ให้โดนหมุนดำ)
- ข้อข้างเคียงที่ยอมรับ: `z-index != auto` ทำให้แถบเป็น stacking context → `mix-blend-mode: screen` ของ `.zoom-hero-glow` ไปผสมกับรูป "ในแถบ" แทน aurora ของ body (ผลลัพธ์เท่าเดิมเพราะรูปทึบคลุมทั้งแถบ)

⚠️ **`.zoom-hero-scrim` (ความสว่างของแบบด์) ต้องอยู่ "ใต้" `.zoom-hero-vignette` เสมอ** — `ScrollZoomHero` render `<div className="zoom-hero-scrim" style={{ background: rgba(0,0,0,dim) }} />` เป็นลูกตัวรองสุดท้าย (ก่อน vignette)
- เหตุผล: slider มีไว้ "หรี่ภาพ" ไม่ใช่ "หรี่เงา" — ถ้า scrim ไปทับ vignette ด้วย ความเข้มของ vignette ที่ช่วยให้การ์ดแถวบนสุดอ่านออกจะถูกตัดทอนตามไปด้วย
- `dim` default `0` → `rgba(0,0,0,0)` = หน้าตาเดิมเป๊ะ (ต้องคงไว้ ไม่งั้นคนที่ยังไม่เคยตั้งจะเห็นแบบด์มืดลงโดยไม่ตั้งใจ) · ช่วง 0–0.85 เหมือนพื้นหลังแอป · `readStoredHeroDim()` ใน `BackgroundPicker` เป็นตัวอ่าน (fallback = `HERO_DIM_DEFAULT`)
- ใน `BackgroundPicker` สลับคีย์ตาม variant: `app → tdapp_bg_dim` (ค่าเริ่มต้น 0.55, slider ปิดจนกว่าจะมีรูป) · `hero → tdapp_hero_dim` (ค่าเริ่มต้น 0, เปิดทันทีเพราะมีภาพเริ่มต้นในตัว → `dimEnabled = cfg.showDim && previewSrc !== null`)
...
**Drag orb = เลนส์หยดน้ำ Fluid Glass (กลางนูน + ขอบบีบ + รุ้งไม่สมมาตร + ขอบบางใส):** `MobileNav.tsx` วาด radial displacement map ลง canvas (`buildLensMap`, smootherstep `S(t)=6t⁵−15t⁴+10t³` ยกกำลัง `P/2=1.0` + ผสมโดมนุ่ม `D(t)=t·(2-t)` สัดส่วน `DOME=0.45` + `P=3.0` (`P/2=1.5`) — `D'(1)=0` ไม่มีรอยหักที่ขอบ, `N=256` + เบลอ 1px ฆ่าขั้นบันได 8-bit, เครื่องหมายลบ = ดึง sample เข้าหาศูนย์กลาง) → `feImage` → `feDisplacementMap` แยกช่องสี R17/G20/B24 ผ่าน `backdrop-filter: url("#dock-glass-lens")` บน `.dock-glass__orb` เท่านั้น — กลางวงหักเหน้อยสุด ~1.25x (ภาพกลางนิ่ง) ไล่แรงขึ้นไปบีบที่ขอบแบบหยดน้ำ บีบแรงเฉพาะแถบขอบแบบหยดน้ำ (ไม่ใช่แว่นขยายทั้งวง) · transition: `feDisplacementMap scale` เริ่ม 0 แล้ว render() ดันเป็น 17/20/24 ตาม alpha แบบ smootherstep ทุกเฟรม (ตั้งผ่าน ref `dispR/G/BRef` — SVG attribute ไม่มี CSS transition) ⇒ เริ่มลากเลนส์ค่อย ๆ นูน / ปล่อยค่อย ๆ ยุบ ไม่ป๊อป (วัดจริงตอนเริ่มลาก 15.49/18.22/21.86 → เต็ม 17/20/24) (ห้ามปน blur/saturate — Chromium ทิ้ง url ทั้งตัว; ห้ามมี backdrop-filter บน `<nav>` หรือบรรพบุรุษ — สร้าง backdrop root ฆ่า displacement) · scale เดิม R14/G18/B22 ดึงไอคอนสว่างหลังวงมาละเลงเป็นวงขาวหนา ~10px (เทียบ D0–D3) ⇒ เคยลดครึ่งเหลือ R7/G9/B11 แล้วบูสต์แรงต่อเนื่อง 3 รอบตามคำขอ "เอฟเฟคยังเบาไป" จนถึง R17/G20/B24 (±8.5/±10/±12px) ขอบสีแยก ~3.5px ชัดแต่ยังไม่เป็นวงขาวหนา · ผู้ใช้ชี้ "ตัวอย่างใสกว่า มีเคลือบสี" ⇒ ลดฝ้าขาวทั้งวงเหลือแค่เคลือบสี รอบแรก: orb พื้น 0.04/0.03, sheen เนื้อใน 0.04/0.02 + inner caustic 83% α 0.12 + วงหลัก α 0.10 + hairline α 0.50, caustic ชั้น opacity 0.5 (พื้น 0.10/0.06/0.04), scatter α 0.14/0.12/0.10, edge-light พีค 0.55, rim 1px α 0.55/0.26/0.58 — รอบ "นูนอีก ใสขึ้นอีก": DOME 0.35→0.5 (กลาง ~1.3x) + ฝ้าลดอีก: orb พื้น 0.025/0.02, sheen เนื้อใน 0.025/0.012, inner caustic α 0.08, วงหลัก α 0.07, caustic ชั้น opacity 0.35 (พื้น 0.07/0.04/0.025), scatter α 0.10/0.09/0.07 (กลางวงใสเห็นฉากหลังบิดเต็ม เหลือรุ้ง/แสงแค่แถบขอบแบบตัวอย่าง) · ผู้ใช้ชี้ "ขอบหยัก + ไล่สีไม่สมูท" ⇒ ต้นตอ 3 จุด: (1) แผนที่ 160px ควอนไทซ์ 255 ขั้น + `min(r,1)^3` หักที่ขอบ + (2) warp octave 2 มีคลื่นถี่บน hairline + (3) mask/gradient เฟดชัน 1% ⇒ แก้: N 160→256 + smootherstep + blur 1px, warp เหลือ octave 1 (0.008/0.01, blur 8, scale 6→4), caustic 0.05/0.06 blur 2.6 scales 3/6/9 → 0.032/0.038 blur 3.5 scales 2/4/6, mask เลนส์ 97/99/99.5/100 → 94/96/98.5/100, hairline/disperse/irid/edge/inner/main เติม stop กลางไล่นุ่มทั้งสองข้าง (เช่น disperse 86.5/89.5/91.5/93.2/95%) · glare ~10 นาฬิกาเอาออกแล้ว (ผู้ใช้สั่ง) แทนด้วยแสงกระเจิงไม่สมมาตร: scatter 3 หย่อม + disperse ตามทิศลาก 2 มิติ (แดงสวนนิ้ว/น้ำเงินตามนิ้ว, `--spx 0.06+0.06sp` แยก ~5px, `--sdx/--sdy` ×5.6 ค้างตอนถือค้างผ่านทิศจุดกด) + irid รุ้ง conic หมุนตาม `--dang` (atan2 ทิศลาก, พีค ~0.64, มาสก์นุ่ม 86→96%) + edge-light ขอบ 92–100% หมุนตาม `--dang` สว่างด้านเดียว · ทิศลากต้องเทียบจุดกด (`startLocalX/Y`) ไม่ใช่จุดพัก · กันล้นขอบจอ: OVER_X 18→8, แนวตั้งแยก OVER_TOP 34 / OVER_BOTTOM 10 · verified รอบเรียบใส (390×844): LEFT7/RIGHT7 o:1 lens:true ทั้งคู่, ครอปซ้ายรอบแรกดำเพราะใช้พิกัดเก่า (x87 y758 ของรันก่อน) ไม่ใช่ตัววง — ถ่ายใหม่ LEFT8 ได้ rect จริง x47 y587 w84 h84 o:1 lens:true, ซูมเห็นขอบกลมเรียบ + ไอคอน Signal แยกสีชัด + กลางวงใส

**วาร์ปภาพในหยดน้ำ (image warp — ไม่ใช่แค่ shift):** แผนที่มี angular wobble `LENS_MAP_WOB=0.14` (พู 3+5 × env 0 กลาง→1 ขอบ — เส้นตรงในวงบิดเป็นคลื่น) + turbulence stage ท้าย `#dock-glass-lens` (`warpRef`, scale ~9 นิ่ง → ~18 ลากเร็ว × ae) · **เลนส์เร่งขอบ:** shift ตำแหน่ง 12/16/22 (ขอบบีบ ±6/±8/±11px) แต่ shift สี B−R 10px (ขอบแยก ~5px) — กลางแยกน้อยสุด ไล่พุ่งที่ขอบแบบไม่เป็นเชิงเส้น (`P=5.0`/`DOME=0.22`, กลาง ~1.1x) · **เลขเวอร์ชันท้ายโฮม:** `v<GIT_SHA>` ตัวเล็กจาง ๆ (`text-[10px] text-slate-600/70`, `aria-hidden`) — `scripts/write-version.js` เขียน short SHA ลง `src/lib/gitVersion.ts` ผ่าน `prebuild`/`predev` (ไฟล์ generated นี้ gitignore ไม่ commit กันค่าล้าหนึ่งคอมมิต) · **หยดน้ำตามโมเมนตัม (smooth morph):** velocity สปริงกรอง low-pass อีกชั้น (`mom.x/y` TAU ~90ms, `mom.ang` shortest-arc TAU ~120ms, render รับ `dt`) — orb/pill/wake `rotate(ang)+scale` ทั้งก้อนตามทิศแรง (orb ยืดสุด ~1.38x, wake หางยาวตามแรง), rim/disp/irid ใช้ local-x + `--dang:0deg` (อยู่ใน orb ที่ rotate แล้ว ไม่บวกมุมซ้ำ) · **กลางซูม+ขอบนุ่ม+กลางใส:** `DOME 0.22→0.5` (กลาง ~1.3x) + wobble taper `(1-t⁴)` + orb/sheen/caustic/scatter ลดฝ้ากลาง + disperse กาง 80%→97% 8 stops + blur 0.6px + rim blur 0.4px + irid mask นุ่ม 84%→97% · **morph ไฮไลท์ capsule<->circle (ไป/กลับเห็น transform ไม่วาร์ป):** `morphT=smootherstep(alpha)` ตัวเดียวกับเลนส์ — pill โตเข้าหาวงกลม 84px (`pillMorphW/H`) ส่วน orb เริ่มจากทรงแคปซูลเท่า pill แล้วค่อยเป็นวงกลม+ยืดตามแรง กลางทางทรงตรงกัน crossfade จึง seamless; **กดเห็นยุบ (~7% press-morph ตั้งแต่ `onDown`)** tap ตรง ๆ สไลด์สปริงไปแท็บใหม่ (peak alpha 0.55, หน่วง navigate ~180ms) / ลากปล่อย morph กลับ (หน่วง ~220ms) แล้วค่อย `location.assign` — `pointercancel` ไม่ navigate; **slop/gate:** ของเหลวติดเมื่อขยับเกิน 10px ใน `onMove`, navigate เฉพาะ `maxDrag>24px` สะกิดโดน snap กลับทันที · **pill เอียงค้างหลังลากเฉียงที่เมนูเดิม (3a1d814):** mom.ang ค้างทิศลากสุดท้ายเพราะนิ่งแล้วคงทิศเดิม — แก้สองชั้น: (1) ปล่อยนิ้ว+ความเร็วต่ำ ดึง mom.ang กลับ 0 องศาแบบนุ่ม (TAU 180ms ให้เห็น transform), (2) มุม pill โดนเกทด้วย morphT+spd (พักนิ่งได้ 0 องศาเป๊ะ ส่วน orb/wake ยังเอียงเต็มตอนลาก) + snap() รีเซ็ต mom.ang=0 · **prerender เป็นกลาง:** `path=null` จน hydrate (static export ไม่มี window) ⇒ HTML แรกไม่มีแท็บ active + pill ซ่อน (`revealed` ใน sync ค่อยเผยเมื่อ DOM มี `aria-current`) — กันทุกหน้า paint "Home ฟ้า" ก่อนหนึ่งเฟรมตอนกดเมนูตรง ๆ (เช่น signal>monitor)
