...
**Drag orb = เลนส์หยดน้ำ Fluid Glass (กลางนิ่ง + ขอบบีบ + รุ้งไม่สมมาตร + ขอบบางใส):** `MobileNav.tsx` วาด radial displacement map ลง canvas (`buildLensMap`, `LENS_MAP_P = 3.0` กลางแบน ขอบชัน, เครื่องหมายลบ = ดึง sample เข้าหาศูนย์กลาง) → `feImage` → `feDisplacementMap` แยกช่องสี R17/G20/B24 ผ่าน `backdrop-filter: url("#dock-glass-lens")` บน `.dock-glass__orb` เท่านั้น — กลางวงภาพนิ่ง ~1x บีบแรงเฉพาะแถบขอบแบบหยดน้ำ (ไม่ใช่แว่นขยายทั้งวง) (ห้ามปน blur/saturate — Chromium ทิ้ง url ทั้งตัว; ห้ามมี backdrop-filter บน `<nav>` หรือบรรพบุรุษ — สร้าง backdrop root ฆ่า displacement) · scale เดิม R14/G18/B22 ดึงไอคอนสว่างหลังวงมาละเลงเป็นวงขาวหนา ~10px (เทียบ D0–D3) ⇒ เคยลดครึ่งเหลือ R7/G9/B11 แล้วบูสต์แรงต่อเนื่อง 3 รอบตามคำขอ "เอฟเฟคยังเบาไป" จนถึง R17/G20/B24 (±8.5/±10/±12px) ขอบสีแยก ~3.5px ชัดแต่ยังไม่เป็นวงขาวหนา · glare ~10 นาฬิกาเอาออกแล้ว (ผู้ใช้สั่ง) แทนด้วยแสงกระเจิงไม่สมมาตร: scatter 3 หย่อม (อุ่น/เย็น/ม่วง α 0.36/0.31/0.24) + disperse ตามทิศลาก 2 มิติ (แดงสวนนิ้ว/น้ำเงินตามนิ้ว, `--spx 0.06+0.06sp` แยก ~5px, `--sdx/--sdy` ×5.6 ค้างตอนถือค้างผ่านทิศจุดกด) + irid รุ้ง conic หมุนตาม `--dang` (atan2 ทิศลาก, พีค ~0.64, มาสก์วงแหวน 87–95.5%) + edge-light ขอบ 93–99% หมุนตาม `--dang` สว่างด้านเดียว (พีค α 1.0) · ทิศลากต้องเทียบจุดกด (`startLocalX/Y`) ไม่ใช่จุดพัก — วัดจริงลากซ้าย/ขวา `dang` −153.4° vs −26.6° (แยกกัน 127°) · ขอบบางใสแต่แรง: rim 1px (α 0.75/0.35/0.78), disperse วงแคบ 88%→พีค 91.5%→จาง 94% (α 0.58/0.33/0.62), sheen hairline 95%→96.5%→98.2% α 0.75 (~1.1px) + inner caustic 83% α 0.24 + วงหลัก α 0.20, caustic ชั้น α 1.0 (พื้น 0.22/0.13/0.08), orb พื้น 0.15/0.10/0.10, rimShift 1.4+17spd (1.4–5.8px), mask เลนส์ทึบถึง 97% เฟดแคบ 99/99.5/100% (เนื้อวงแสดงภาพบิดทั้งวง คงขอบคม ~1px) · กันล้นขอบจอ: OVER_X 18→8, แนวตั้งแยก OVER_TOP 34 / OVER_BOTTOM 10 · verified บูสต์รอบ 3 (390×844): LEFT5 o:1 x87 y758 dang −168.7° / RIGHT5 o:1 x171 y571 dang 82.5°, ซูมขวาเห็นรุ้งรอบวง + ไอคอนแยกสีชัด
