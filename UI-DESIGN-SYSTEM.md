...
**วาร์ปภาพในหยดน้ำ (image warp — ไม่ใช่แค่ shift):** แผนที่มี angular wobble `LENS_MAP_WOB=0.08` (พู 3+5 × env 0 กลาง→1 ขอบ — เส้นตรงในวงบิดเป็นคลื่น) + turbulence stage ท้าย `#dock-glass-lens` (`warpRef`, scale ~7 นิ่ง → ~14 ลากเร็ว × ae) · **เลขเวอร์ชันท้ายโฮม:** `v<GIT_SHA>` ตัวเล็กจาง ๆ (`text-[10px] text-slate-600/70`, `aria-hidden`) — `scripts/write-version.js` เขียน short SHA ลง `src/lib/gitVersion.ts` ผ่าน `prebuild`/`predev` (ไฟล์ generated นี้ gitignore ไม่ commit กันค่าล้าหนึ่งคอมมิต)
