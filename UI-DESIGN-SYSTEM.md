...
**Drag orb = เลนส์หยดน้ำ Fluid Glass (กลางนิ่ง + ขอบบีบ + รุ้งไม่สมมาตร + ขอบบางใส):** `MobileNav.tsx` วาด radial displacement map ลง canvas (`buildLensMap`, `LENS_MAP_P = 3.0` กลางแบน ขอบชัน, เครื่องหมายลบ = ดึง sample เข้าหาศูนย์กลาง) → `feImage` → `feDisplacementMap` แยกช่องสี R7/G9/B11 ผ่าน `backdrop-filter: url("#dock-glass-lens")` บน `.dock-glass__orb` เท่านั้น — กลางวงภาพนิ่ง ~1x บีบแรงเฉพาะแถบขอบแบบหยดน้ำ (ไม่ใช่แว่นขยายทั้งวง) (ห้ามปน blur/saturate — Chromium ทิ้ง url ทั้งตัว; ห้ามมี backdrop-filter บน `<nav>` หรือบรรพบุรุษ — สร้าง backdrop root ฆ่า displacement) · scale เดิม R14/G18/B22 ดึงไอคอนสว่างหลังวงมาละเลงเป็นวงขาวหนา ~10px (เทียบ D0–D3) ⇒ ลดครึ่งเหลือ R7/G9/B11 (±3.5/±4.5/±5.5px) วงใสเหลือรุ้งขอบบาง · glare ~10 นาฬิกาเอาออกแล้ว (ผู้ใช้สั่ง) แทนด้วยแสงกระเจิงไม่สมมาตร: scatter 3 หย่อมจาง (อุ่น/เย็น/ม่วง, บูสต์เห็นบนพื้นเข้ม α 0.13/0.11/0.085) + disperse ตามทิศลาก 2 มิติ (แดงสวนนิ้ว/น้ำเงินตามนิ้ว, `--sdx/--sdy` ≤ 2.5px ค้างตอนถือค้างผ่านทิศจุดกด) + irid รุ้ง conic หมุนตาม `--dang` (atan2 ทิศลาก, ด้านรับแสงชัด ตรงข้ามจางเกือบ 0, มาสก์วงแหวน 87–95.5%) + edge-light ขอบ 93–99% หมุนตาม `--dang` สว่างด้านเดียว (α 0.4 พีค) · ทิศลากต้องเทียบจุดกด (`startLocalX/Y`) ไม่ใช่จุดพัก — วัดจริงลากซ้าย/ขวา `dang` −153.4° vs −26.6° (แยกกัน 127°, เดิมเทียบ rest ได้ −11.1° vs −7.6° แทบไม่ต่าง) · ขอบบางใส: rim 1px (α 0.30/0.14/0.32 บูสต์เห็นบนพื้นเข้ม), disperse วงแคบ 88%→พีค 91.5%→จาง 94% (α 0.22/0.12/0.24), sheen hairline 95%→96.5%→98.2% α 0.30 (~1.1px), inner caustic 83% α 0.08, mask เลนส์ทึบถึง 97% เฟดแคบ 99/99.5/100% (เนื้อวงแสดงภาพบิดทั้งวง คงขอบคม ~1px) · กันล้นขอบจอ: OVER_X 18→8, แนวตั้งแยก OVER_TOP 34 / OVER_BOTTOM 10
