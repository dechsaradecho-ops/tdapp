...
**Drag orb = เลนส์หยดน้ำ Fluid Glass (กลางนิ่ง + ขอบบีบ + รุ้งไม่สมมาตร + ขอบบางใส):** `MobileNav.tsx` วาด radial displacement map ลง canvas (`buildLensMap`, `LENS_MAP_P = 3.0` กลางแบน ขอบชัน, เครื่องหมายลบ = ดึง sample เข้าหาศูนย์กลาง) → `feImage` → `feDisplacementMap` แยกช่องสี R7/G9/B11 ผ่าน `backdrop-filter: url("#dock-glass-lens")` บน `.dock-glass__orb` เท่านั้น — กลางวงภาพนิ่ง ~1x บีบแรงเฉพาะแถบขอบแบบหยดน้ำ (ไม่ใช่แว่นขยายทั้งวง) (ห้ามปน blur/saturate — Chromium ทิ้ง url ทั้งตัว; ห้ามมี backdrop-filter บน `<nav>` หรือบรรพบุรุษ — สร้าง backdrop root ฆ่า displacement) · scale เดิม R14/G18/B22 ดึงไอคอนสว่างหลังวงมาละเลงเป็นวงขาวหนา ~10px (เทียบ D0–D3) ⇒ ลดครึ่งเหลือ R7/G9/B11 (±3.5/±4.5/±5.5px) วงใสเหลือรุ้งขอบบาง · glare ~10 นาฬิกาเอาออกแล้ว (ผู้ใช้สั่ง) แทนด้วยแสงกระเจิงไม่สมมาตร: scatter 3 หย่อมจาง (อุ่น/เย็น/ม่วง, α ≤ 0.055) + disperse ตามทิศลาก 2 มิติ (แดงสวนนิ้ว/น้ำเงินตามนิ้ว, `--sdx/--sdy` ≤ 2.5px ค้างตอนถือค้างผ่าน target-rest) + irid รุ้ง conic 16–80% รอบวง (ด้านรับแสงชัด ตรงข้ามจางเกือบ 0) + edge-light ขอบ 93–99% สว่างด้านเดียว · ขอบบางใส: rim 1px α ≤ 0.17, disperse วงแคบ 88%→พีค 91.5%→จาง 94%, hairline 94%→96.8%→98.6% α 0.09, mask เลนส์ทึบถึง 92% เฟดใน 8% สุดท้าย · กันล้นขอบจอ: OVER_X 18→8, แนวตั้งแยก OVER_TOP 34 / OVER_BOTTOM 10
